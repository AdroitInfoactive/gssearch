...
if (isset($_POST['btnsbmt_chng_pwd']) && (trim($_POST['btnsbmt_chng_pwd']) == 'Submit') && isset($_POST['txtchngpwd_oldpswd']) && (trim($_POST['txtchngpwd_oldpswd']) != '') && isset($_POST['txtchngpwd_nwpswd']) && (trim($_POST['txtchngpwd_nwpswd']) != '') && isset($_POST['txtchngpwd_nwcnfpswd']) && (trim($_POST['txtchngpwd_nwcnfpswd']) != '')) {
  ...
  $oldpswd = glb_func_chkvl($_POST['txtchngpwd_oldpswd']);
  $pswd = glb_func_chkvl($_POST['txtchngpwd_nwpswd']);
  $ipadrs = $_SERVER['REMOTE_ADDR'];
  $opwd = md5($oldpswd);
  $pwd = md5($pswd);
  $dt = date('Y-m-d h:i:s');
  $sts = 'a';
  $sqrymbr_mst = "SELECT mbrm_id from mbr_mst where mbrm_id = '$mbrid' and mbrm_pwd = '$opwd'";
  $srsmbr_mst = mysqli_query($conn, $sqrymbr_mst);
  $cntmbr_mst = mysqli_num_rows($srsmbr_mst);
  if ($cntmbr_mst < 1) {
    ?>
    <script language="javascript" type="text/javascript">
      alert("Old Password donot match with our records. Please try with correct credentials.");
      location.href = "<?php echo $rtpth . 'home' ?>";
    </script>
    <?php
    // $greg_msg = "Duplicate email id, account not created";
  } else {
    $uqry_mbr_mst = "UPDATE mbr_mst set mbrm_pwd = '$pwd' where mbrm_id = '$mbrid' and mbrm_pwd = '$opwd'";
      ...
      $body = "<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML 4.01//EN' 'http://www.w3.org/TR/html4/strict.dtd'>
				...
				<table width='600'  border='0' align='center' cellpadding='6' cellspacing='0'>
					  <tr>
						<td><p><br>
						  Dear $name, 
						  <p>Your Account Password has been changed Successfully. Please find the details below.</p>
