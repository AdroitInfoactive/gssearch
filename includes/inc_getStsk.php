...
if(isset($_REQUEST['mncatval']) && (trim($_REQUEST['mncatval']) != ""))
{
	...
	$sqryprodcat_mst = "SELECT prodcatm_id,prodcatm_name from prodcat_mst where prodcatm_sts = 'a' and prodcatm_subtopicsm_id = $prodmncatid group by prodcatm_id order by prodcatm_name";
...
if (isset($_REQUEST['topicval']) && (trim($_REQUEST['topicval']) != "")) {
	// creating Drop Down for sub category
	$result = "";
	$topicid = glb_func_chkvl($_REQUEST['topicval']);
 	$sqrytopic_mst = "SELECT subtopicsm_id,subtopicsm_name from subtopics_mst where subtopicsm_sts = 'a' and subtopicsm_topicsm_id = $topicid group by subtopicsm_id order by subtopicsm_prty asc";
	//  echo $sqrytopic_mst; exit;
	$srstopic_mst = mysqli_query($conn, $sqrytopic_mst);
	$cntprodcat_inc = mysqli_num_rows($srstopic_mst);
	$dispstr = "";
	?>
	<option value disabled selected>Select sub Topic</option>
	<?php
	while ($prg = mysqli_fetch_array($srstopic_mst)) { ?>
		<option value="<?php echo $prg["subtopicsm_id"]; ?>"><?php echo $prg["subtopicsm_name"]; ?></option>
		<?php
	}
}
// --------------------------------------------------------------------------
// ------------------------- To get related exec sub categories-------------------
if (isset($_REQUEST['execcatval']) && (trim($_REQUEST['execcatval']) != "")) {
	// creating Drop Down for sub category
	$result = "";
	$execcatid = glb_func_chkvl($_REQUEST['execcatval']);
	$sqryexeccat_mst = "SELECT exect_scatm_id,exect_scatm_name from exect_scat_mst where exect_scatm_sts = 'a' and exect_scatm_catm_id = $execcatid group by exect_scatm_id order by exect_scatm_prty asc";
	// echo $sqryexeccat_mst; exit;
	$srsexeccat_mst = mysqli_query($conn, $sqryexeccat_mst);
	$cntprodcat_inc = mysqli_num_rows($srsexeccat_mst);
	$dispstr = "";
	?>
	<option value disabled selected>Select sub Category</option>
	<?php
	while ($prg = mysqli_fetch_array($srsexeccat_mst)) { ?>
		<option value="<?php echo $prg["exect_scatm_id"]; ?>"><?php echo $prg["exect_scatm_name"]; ?></option>
		<?php
	}
}
// --------------------------------------------------------------------------
...
// 	$sqry_admtyp = "SELECT subtopicsm_id,subtopicsm_name from subtopics_mst where subtopicsm_id=$adm_typ_id and  subtopicsm_sts = 'a'";
	
// 	$srsadmtyp = mysqli_query($conn,$sqry_admtyp) or die(mysqli_error($conn));
// 	$srowslctd = mysqli_fetch_assoc($srsadmtyp);
// 	$admtypid = $srowslctd['subtopicsm_id'];
// 	$admtypnm = $srowslctd['subtopicsm_name'];
...
// -------------------------------------------------------------- generate bod and mot ----------------------------------------------------------------------------------
if (isset($_REQUEST['gnrttyp']) && (trim($_REQUEST['gnrttyp']) != "")) {
	$result = "";
	$gnrttyp = trim($_REQUEST['gnrttyp']);
	if ($gnrttyp == "b" || $gnrttyp == "m")
	{
		$sqry_gnrt_vntnt = "SELECT bodm_id,bodm_name, bodm_desc, bodm_dimgnm FROM bod_mst where bodm_sts = 'a' and bodm_typ = '$gnrttyp' order by bodm_prty asc";
		$srsgnrt = mysqli_query($conn, $sqry_gnrt_vntnt) or die(mysqli_error($conn));
		$cntgnrt = mysqli_num_rows($srsgnrt);
		if ($cntgnrt > 0)
		{
			while ($srowgnrt = mysqli_fetch_assoc($srsgnrt))
			{
				$gnrt_id = $srowgnrt['bodm_id'];
				$gnrt_nm = $srowgnrt['bodm_name'];
				$gnrt_desc = $srowgnrt['bodm_desc'];
				$gnrt_dimgnm = $srowgnrt['bodm_dimgnm'];
				$gnrt_img_pth = $u_gbod_fldnm.$gnrt_dimgnm;
				if ($gnrt_dimgnm != "") {
					$gnrtimgpth = $rtpth . $gnrt_img_pth;
				} else {
					$gnrtimgpth = $rtpth . $u_gbod_fldnm . 'no-image.png';
				}
				$result .= "<div class='text-content'>
					<div class='row'>
						<div class='col-lg-4 col-md-6' data-sal='slide-up' data-sal-duration='800' data-sal-delay='300'>
							<div class='sc-team-pages-content-box text-center mb-4'>
								<div class='sc-page-item'>
									<a href='#'><img src='$gnrtimgpth' alt='$gnrt_nm' /></a>
									<div class='sc-slider-text'>
										<h4>
											<a class='title' href='#'>$gnrt_nm</a>
										</h4>
										<span class='sub-title'>$gnrt_desc</span>
									</div>
								</div>
							</div>
						</div>
	...
	{
		$gnrt_typ_expld = explode("-", $gnrttyp);
		$gnrt_typ = $gnrt_typ_expld[1];
		$sqry_crs = "SELECT Coursm_id, Coursm_name,Coursm_img,Coursm_typ from cours_mst where Coursm_typ='$gnrt_typ' and Coursm_sts='a' order by Coursm_prty asc";
		$srscrs = mysqli_query($conn, $sqry_crs) or die(mysqli_error($conn));
		$cntcrs = mysqli_num_rows($srscrs);
		if ($cntcrs > 0)
		{
			while ($srowcrs = mysqli_fetch_assoc($srscrs))
			{
				$crs_id = $srowcrs['Coursm_id'];
				$crs_nm = $srowcrs['Coursm_name'];
				$crs_img = $srowcrs['Coursm_img'];
				$crs_typ = $srowcrs['Coursm_typ'];
				$crs_img_pth = $u_gcrs_fldnm . $crs_img;
				if ($crs_img != "")
				{
					$crsimgpth = $rtpth . $crs_img_pth;
				}
				else {
					$crsimgpth = $rtpth . $u_gcrs_fldnm . 'no-image.png';
				}
				$result .= "<div class='col-lg-6 col-md-6'><div class='sc-services-style3 d-flex justify-content-between sc-mb-80'>
            <div class='sc-service-image'>
              <img src='$crsimgpth' alt='Service' />
            </div>
            <div class='sc-service-text'>
              <h4 class='service-title'><a href='".$rtpth."category_detail.php?grad_typ=$crs_typ&crs_id=$crs_id'>$crs_nm</a></h4>
              <div class='sc-service-btn'>
                <a class='sc-transparent-btn' href='<?php echo $rtpth; ?>msc-in-hospitality-administration.php'>Read
                  More</a>
              </div>
              <div class='sc-team-btn mt-3 d-flex'>
                <a class='sc-primary-btn-three ' href='#' data-bs-toggle='modal' data-bs-target='#applyNow'>Apply
                  Now</a>
              </div>
            </div>
          </div>
...
// 	$sqryprodcat_mst = "SELECT prodcatm_id, prodcatm_name from prodcat_mst where prodcatm_sts = 'a' and prodcatm_subtopicsm_id = $prodmcatid group by prodcatm_id order by prodcatm_prty";
// 	// echo $sqryprodscat_mst; exit;
// 	$srsprodcat_mst = mysqli_query($conn,$sqryprodcat_mst);
// 	$cntprodcat_inc = mysqli_num_rows($srsprodcat_mst);
// 	$dispstr = "";
...
if(isset($_REQUEST['prodmcat']) && (trim($_REQUEST['prodmcat']) != ""))
{
	...
	$sqryprodscat_mst = "SELECT prodcatm_id, prodcatm_name from prodcat_mst where prodcatm_sts = 'a' and prodcatm_subtopicsm_id = $prodcatid group by prodcatm_id order by prodcatm_prty";
	// echo $sqryprodscat_mst; exit;
	$srsprodscat_mst = mysqli_query($conn,$sqryprodscat_mst);
	$cntprodscat_inc = mysqli_num_rows($srsprodscat_mst);
	$dispstr = "";
	...
	<div class="row mb-2 mt-2">
		...
		<div class="col-sm-9">
			<select name="lstprodcat" id="lstprodcat" class="form-control"  onchange="get_sub_cat();" >
			 ...
			 $sqryprodcat_mst = "SELECT prodcatm_id, prodcatm_name from prodcat_mst where prodcatm_sts = 'a' and prodcatm_subtopicsm_id = $prodmncatid group by prodcatm_id order by prodcatm_prty";
				 //echo $sqryprodcat_mst; exit;
				$srsprodcat_mst = mysqli_query($conn,$sqryprodcat_mst);
